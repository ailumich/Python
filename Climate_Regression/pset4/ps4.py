...
def generate_models(x, y, degs):
    ...
    result = []
    for i in degs:
        result.append(numpy.polyfit(x,y,i))
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in models:
        ...
        estyval = []
        for elt in x:
            estyval.append(a*elt+b)
        pylab.figure(figurecount)
        pylab.plot(x,y, 'bo', label = 'Data Points')
        pylab.plot(x, estyval,'r', label = 'Estimate')
        pylab.title('Temperature Change over Time \n R^2 ='+str(r_squared(y, estyval)))
        pylab.xlabel('Year')
