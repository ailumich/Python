...
def loadWords():
    ...
    Depending on list size, function can run for a while
    """
...
def getWordScore(word, n):
    ...
    for letter in word:
        ...
        word_pts += temp
    if len(word) == n: # does not check if all letters are in hand
        return word_pts*mult+50
    else:
        return word_pts*mult
...
def dealHand(n):
    ...
    """
    hand={}
    numVowels = n // 3  #returns only INT portion
    ...
    while calculateHandlen(hand) > 0: # As long as there are still letters left in the hand:
        ...
        if word == '.': # If the input is a single period
            ...
            if isValidWord(word, hand, wordList) == False: # If the word is not valid
                ...
                total_score += score 
                print('"{}" earned {} points. Total: {} points. \n'.format(word, score, total_score)) # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
